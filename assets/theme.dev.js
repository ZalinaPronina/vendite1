...
(function (bodyScrollLock, themeCurrency, themeImages, themeAddresses, Sqrl, Flickity, FlickityFade, Rellax, ellipsed, AOS) {
  ...
  class CartDrawer {
    ...
    getCart() {
      fetch('/cart.js')
        ...
        .then((response) => {
          ...
          /**
           * Update cart item count if deposit product is added in cart.
          */
          if (window.theme.deposit) {
            response.items.forEach((item) => {
              if (item.product_id === window.theme.deposit) {
                itemCount = itemCount - item.quantity;
              };
            });
          };

          this.updateCounter(itemCount);
    ...
    addToCart(data, quickAddHolder = null, button = null, json = null) {
      ...
      const depositProduct = formData.get('deposit');
    ...
    updateCart(updateData = {}, holder = null, valueIsEmpty = false) {
      ...
      fetch('/cart.js')
        ...
        .then((response) => {
          ...
          const targetCartItem = response.items.find(matchKeys);
          const associatedDepositItem = response.items.find((item) => {
            return item.properties ?
              item.properties['Product'] === targetCartItem.product_title :
              false;
          });
          ...
          if (associatedDepositItem) {
            const updates = {
              [targetCartItem.key]: settedQuantity,
              [associatedDepositItem.key]: settedQuantity,
            };
        ...
        .then((response) => {
          ...
          */
          if (window.theme.deposit) {
            response.items.forEach((item) => {
              if (item.product_id === window.theme.deposit) {
                newCount = newCount - item.quantity;
              };
            });
          };
  ...
  class RangeSlider {
    constructor(section) {
      this.container = section.container;
      this.slider = section.querySelector(selectors$Range.rangeSlider);
      ...
      if (this.slider) {
        ...
        this.touchLeft = this.slider.querySelector(selectors$Range.rangeDotLeft);
        this.touchRight = this.slider.querySelector(selectors$Range.rangeDotRight);
        this.lineSpan = this.slider.querySelector(selectors$Range.rangeLine);

        // get some properties
        this.min = parseFloat(this.slider.getAttribute(selectors$Range.dataMin));
        this.max = parseFloat(this.slider.getAttribute(selectors$Range.dataMax));
    ...
    init() {
      ...
      if (this.slider.hasAttribute(selectors$Range.dataMinValue)) {
        defaultMinValue = parseFloat(this.slider.getAttribute(selectors$Range.dataMinValue));
      }
      let defaultMaxValue = this.max;

      if (this.slider.hasAttribute(selectors$Range.dataMaxValue)) {
        defaultMaxValue = parseFloat(this.slider.getAttribute(selectors$Range.dataMaxValue));
      }
      ...
      if (this.slider.getAttribute(selectors$Range.dataStep)) {
        this.step = Math.abs(parseFloat(this.slider.getAttribute(selectors$Range.dataStep)));
      }
    ...
    setMinValue(minValue) {
      ...
      this.slider.setAttribute(selectors$Range.dataMinValue, minValue);
    }
    ...
    setMaxValue(maxValue) {
      ...
      this.lineSpan.style.marginLeft = this.touchLeft.offsetLeft + 'px';
      this.lineSpan.style.width = this.touchRight.offsetLeft - this.touchLeft.offsetLeft + 'px';
      this.slider.setAttribute(selectors$Range.dataMaxValue, maxValue);
    }
    ...
    onMove(event) {
      ...
      if (this.slider.getAttribute('on-change')) {
        ...
        fn(this.slider.getAttribute(selectors$Range.dataMinValue), this.slider.getAttribute(selectors$Range.dataMaxValue));
      }

      this.onChange(this.slider.getAttribute(selectors$Range.dataMinValue), this.slider.getAttribute(selectors$Range.dataMaxValue));
    }
    ...
    onStop() {
      ...
      // call did changed
      this.onChanged(this.slider.getAttribute(selectors$Range.dataMinValue), this.slider.getAttribute(selectors$Range.dataMaxValue));
    }

    onChange(min, max) {
      const rangeHolder = this.slider.closest(selectors$Range.rangeHolder);
      if (rangeHolder) {
        const priceMin = rangeHolder.querySelector(selectors$Range.priceMin);
        const priceMax = rangeHolder.querySelector(selectors$Range.priceMax);
    ...
    onChanged(min, max) {
      if (this.slider.hasAttribute(selectors$Range.dataFilterUpdate)) {
  ...
  class FiltersForm {
    constructor(section) {
      ...
      this.sidebar = this.container.querySelector(selectors$Filters.collectionSidebar);
      this.form = this.container.querySelector(selectors$Filters.form);
      this.sort = this.container.querySelector(`[${selectors$Filters.sort}]`);
      this.productsContainer = this.container.querySelector(selectors$Filters.productsContainer);
      this.collectionNav = this.container.querySelector(selectors$Filters.collectionNav);
      this.init();
    }
      ...
      if (this.form) {
        ...
        this.sidebar.addEventListener(
          ...
          debounce((e) => {
            ...
            if (type === selectors$Filters.input || type === selectors$Filters.select || type === selectors$C.label || type === selectors$C.textarea) {
              if (this.form && typeof this.form.submit === 'function') {
                const priceMin = this.form.querySelector(selectors$Filters.priceMin);
                const priceMax = this.form.querySelector(selectors$Filters.priceMax);
                if (priceMin && priceMax) {
                  if (target.hasAttribute(selectors$Filters.priceMinValue) && !priceMax.value) {
                    priceMax.value = priceMax.placeholder;
                  } else if (target.hasAttribute(selectors$Filters.priceMaxValue) && !priceMin.value) {
    ...
    filterUpdateFromUrl(e) {
      const target = e.target;
      if (target.matches(selectors$Filters.filterUpdateUrlButton) || (target.closest(selectors$Filters.filterUpdateUrlButton) && target)) {
        e.preventDefault();
        const button = target.matches(selectors$Filters.filterUpdateUrlButton) ? target : target.closest(selectors$Filters.filterUpdateUrlButton);
    ...
    submitForm(e, replaceHref = '') {
      this.sort = this.container.querySelector(`[${selectors$Filters.sort}]`);
      const sortValue = this.sort ? this.sort.getAttribute(selectors$Filters.sort) : '';
      if (!e || (e && e.type !== 'popstate')) {
        if (replaceHref === '') {
          ...
          if (this.form) {
            ...
            const rangeMin = this.form.querySelector(selectors$Filters.rangeMin);
            const rangeMax = this.form.querySelector(selectors$Filters.rangeMax);
            const rangeMinDefaultValue = rangeMin && rangeMin.hasAttribute(selectors$Filters.rangeMinDefault) ? rangeMin.getAttribute(selectors$Filters.rangeMinDefault) : '';
            const rangeMaxDefaultValue = rangeMax && rangeMax.hasAttribute(selectors$Filters.rangeMaxDefault) ? rangeMax.getAttribute(selectors$Filters.rangeMaxDefault) : '';
      ...
      if (this.productsContainer) {
        ...
        fetch(`${window.location.pathname}${window.location.search}`)
          .then((response) => response.text())
          .then((data) => {
            this.productsContainer.innerHTML = new DOMParser().parseFromString(data, 'text/html').querySelector(selectors$Filters.productsContainer).innerHTML;

            if (this.sidebar) {
              this.sidebar.innerHTML = new DOMParser().parseFromString(data, 'text/html').querySelector(selectors$Filters.collectionSidebar).innerHTML;

              const activeFiltersCountContainer = this.sidebar.querySelector(`[${selectors$Filters.activeFiltersCount}]`);
              const activeFiltersContainer = this.container.querySelectorAll(selectors$Filters.activeFilters);
              if (activeFiltersCountContainer && activeFiltersContainer.length) {
                const activeFiltersCount = parseInt(activeFiltersCountContainer.getAttribute(selectors$Filters.activeFiltersCount));
            ...
            //Init Quick
            this.productsContainer.querySelectorAll(`[${selectors$h.elements.productGrid}]`).forEach((item) => {
            ...
            console.log(this.collectionNav)
    ...
    updateRange(e) {
      if (this.form && typeof this.form.submit === 'function') {
        ...
        if (checkElements && rangeMin.hasAttribute(selectors$Filters.rangeMinValue) && rangeMax.hasAttribute(selectors$Filters.rangeMaxValue)) {
          const priceMinValue = parseInt(priceMin.placeholder);
          const priceMaxValue = parseInt(priceMax.placeholder);
          const rangeMinValue = parseInt(rangeMin.getAttribute(selectors$Filters.rangeMinValue));
          const rangeMaxValue = parseInt(rangeMax.getAttribute(selectors$Filters.rangeMaxValue));
  ...
  class Collection {
    constructor(section) {
      this.container = section.container;
      this.sort = this.container.querySelector(`[${selectors$Collection.dataSort}]`);
      this.sortButton = this.container.querySelector(selectors$Collection.sortButton);
      this.sortLinks = this.container.querySelectorAll(selectors$Collection.sortLinks);
      this.collectionSidebar = this.container.querySelector(selectors$Collection.collectionSidebar);
      this.collectionSidebarCloseButtons = this.container.querySelectorAll(selectors$Collection.collectionSidebarCloseButton);
      this.groupTagsButton = this.container.querySelector(selectors$Collection.groupTagsButton);
      this.collectionNav = this.container.querySelector(selectors$Collection.collectionNav);
      this.showMoreOptions = this.container.querySelectorAll(selectors$Collection.showMoreOptions);
      this.underlay = this.container.querySelector(selectors$Collection.underlay);
      this.swatches = this.container.querySelectorAll(`[${selectors$Collection.swatch}]`);
      this.accessibility = a11y;
      ...
      if (this.showMoreOptions) {
        this.showMoreOptions.forEach((element) => {
          element.addEventListener('click', (event) => {
            ...
            element.parentElement.previousElementSibling.querySelectorAll(selectors$Collection.linkHidden).forEach((link, index) => {
            ...
            const collectionSidebarSlideOut = this.container.querySelector(selectors$Collection.collectionSidebarSlideOut);
    ...
    sortActions(link, submitForm = true) {
      const sort = link ? link.getAttribute(selectors$Collection.sortValue) : '';
      this.sort.setAttribute(selectors$Collection.dataSort, sort);

      const sortButtonText = this.sort.querySelector(selectors$Collection.sortButtonText);
    ...
    onSortCheck(e) {
      ...
      if (window.location.search.includes('sort_by')) {
        ...
        for (const [key, val] of urlParams.entries()) {
          const linkSort = this.sort.querySelector(`[${selectors$Collection.sortValue}="${val}"]`);
    ...
    initSort() {
      ...
      if (this.sortButton) {
        this.sortButton.addEventListener('click', () => {
          ...
          if (window.innerWidth < 767 && isFiltersSidebarOpen) {
            this.hideSidebar();
          }
        });
      }
    ...
    showSidebarCallback() {
      const collectionSidebarSlider = this.container.querySelector(selectors$Collection.collectionSidebarSlider);
      const collectionSidebarSlideOut = this.container.querySelector(selectors$Collection.collectionSidebarSlideOut);
      ...
      if (collectionSidebarSlideOut === null) {
        if (!window.innerWidth < 767 && isScrollLocked) {
          this.accessibility.removeTrapFocus();
          document.dispatchEvent(new CustomEvent('theme:scroll:unlock', {bubbles: true}));
        }
      }

      if (window.innerWidth < 767 || collectionSidebarSlideOut !== null) {
        if (collectionSidebarSlideOut) {
          this.accessibility.trapFocus(this.collectionSidebar, {
            elementToFocus: this.collectionSidebar.querySelector(selectors$Collection.collectionSidebarCloseButton),
          });
        }
  ...
  const collectionSection = {
    onLoad() {
      sections$Col[this.id] = new Collection(this);
    },
    onUnload() {
      sections$Col[this.id].onUnload();
    },
  };
  ...
  class ProductAddForm {
    constructor(section) {
      ...
      this.errorContainer = this.container.querySelector(selectors$G.formErrorsContainer);
      this.quantityField = this.container.querySelector(selectors$G.quantityField);
    ...
    init() {
      ...
      const variantsElemJSON = this.container.querySelector(selectors$G.variantsJson);
      if (productElemJSON) {
        productJSON = productElemJSON.innerHTML;
      }
      if (variantsElemJSON) {
        variantsJSON = variantsElemJSON.innerHTML;
      }
      if (productJSON) {
      ...
      if (variantsJSON) {
        this.variantsJSON = JSON.parse(variantsJSON);
      } else {
        console.error('Missing variants JSON');
      }

      document.addEventListener('popout:updateValue', this.checkInventory.bind(this))
      ...
      this.document.addEventListener('click', (event) => {
        ...
        if (clickedElement.matches(selectors$G.formCloseError) || clickedElement.closest(selectors$G.formCloseError)) {
          ...
          this.errorContainer.classList.remove(classes$l.visible);
        }
      });
    }
    ...
    checkInventory() {
      const error = {
        message: "Warenkorb-Fehler",
        description: "Alle {{quantity}} {{title}} befinden sich in deinem Warenkorb."
      }

      if (this.variantsJSON) {
        const currentVariantId = parseInt(this.container.querySelector(selectors$e.idInput).value, 10)
        const currentVariant = this.variantsJSON.find((variant) => variant.id === currentVariantId)

        if (currentVariant) {
          const currentInventoryQuantity = currentVariant.inventory_quantity
          const currentQuantity = parseInt(this.quantityField.value, 10)

          if (this.errorContainer) {
            if (currentQuantity > currentInventoryQuantity) {
              const description = error.description.replace('{{quantity}}', currentInventoryQuantity).replace('{{title}}', this.productJSON.title)

              this.errorContainer.innerHTML = `<div class="errors">${error.message}: ${description}<span class="errors__close" data-close-error><svg aria-hidden="true" focusable="false" role="presentation" class="icon icon-close-thin" viewBox="0 0 27 27"><g stroke="#979797" fill="none" fill-rule="evenodd" stroke-linecap="square"><path d="M.5.5l26 26M26.5.5l-26 26"></path></g></svg></span></div>`;

              this.errorContainer.classList.add(classes$l.visible);
            } else {
              this.errorContainer.classList.remove(classes$l.visible);
            }
            // Eva code            	
            if (currentQuantity >= currentInventoryQuantity) {
              document.getElementById("data-quantity-plus").disabled = true;
            } else {
              document.getElementById("data-quantity-plus").disabled = false;
            }
          }
        }
      }
  ...
  const ImageWithTextSection = {
    onLoad() {
      sections$im[this.id] = new ImageWithText(this);
    },
  };
  ...
  class SearchPopdown extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this.popdown.addEventListener('transitionend', (event) => {
        if (event.propertyName == 'visibility' && this.popdownContainer.hasAttribute('open') && this.popdownContainer.getAttribute('open') == 'false') {
          ...
          document.querySelector('body').classList.remove('body-open-search');
        }
      });
    }
    ...
    onBodyClick(event) {
      if (!this.contains(event.target) || event.target.hasAttribute(attributes$i.popdownUnderlay)){
        this.close();
        document.querySelector('body').classList.remove('body-open-search');
      } 
    }
    ...
    open(event) {
      ...
      document.body.classList.add('body-open-search');
      console.log('add')
  ...
  class CollapsibleElements extends HTMLElement {
    constructor() {
      ...
      this.collapsibles = this.querySelectorAll(selectors$Collapsi.collapsible);
      this.single = this.hasAttribute(attributes$k.single);
    }

    connectedCallback() {
      this.collapsibles.forEach((collapsible) => {
        const trigger = collapsible.querySelector(selectors$Collapsi.trigger);
        const body = collapsible.querySelector(selectors$Collapsi.body);
    ...
    open(collapsible) {
      ...
      const body = collapsible.querySelector(selectors$Collapsi.body);
      const content = collapsible.querySelector(selectors$Collapsi.content);
    ...
    onCollapsibleClick(event) {
      ...
      const trigger = event.target.matches(selectors$Collapsi.trigger) ? event.target : event.target.closest(selectors$Collapsi.trigger);
      const collapsible = trigger.closest(selectors$Collapsi.collapsible);
